 ...
 *   数字 / 字符串 / 布尔 / 路径（before.x · after.x · params.x · context.x · object.x）
 *   算术 + - * / % ｜ 比较 == != < <= > >= ｜ 逻辑 and or not ｜ 括号
 *   函数：abs / min / max
 * 任何语法错误、未知路径、类型错误 → 抛 FenceEvalError；判定器按 block 处理（E2.1 宁可错杀）
 */
...
function tokenize(src: string): Token[] {
  ...
  while (i < src.length) {
    ...
    if (/[A-Za-z_]/.test(ch)) {
      ...
      else if (["abs", "min", "max"].includes(word)) tokens.push({ t: "path", v: [word] }); // 函数名在 parser 判别
...
function parse(tokens: Token[]): Ast {
  ...
  const parsePrimary = (): Ast => {
    ...
    if (t.t === "path") {
      if (t.v.length === 1 && ["abs", "min", "max"].includes(t.v[0]!) && peek()?.t === "op" && peek()!.v === "(") {
        eat();
        const args: Ast[] = [];
        if (!(peek()?.t === "op" && peek()!.v === ")")) {
          args.push(parseOr());
...
function evalAst(ast: Ast, scope: EvalScope): unknown {
  switch (ast.k) {
    ...
    case "path": return resolvePath(scope, ast.v);
    case "un": {
