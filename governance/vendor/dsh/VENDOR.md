...
| 锁定版本 | `0.1.0-rc.8`（npm 发布时间 2026-08-19T15:41:29Z；由 rc.6 于 2026-08-20 升级） |
| 来源 | `https://registry.npmjs.org/@deepseek-ai/dsh/-/dsh-0.1.0-rc.8.tgz` |
| integrity（入库时实测） | `sha512-VQU5NlomrKLRgcXuOf+sxWFvqxPA8q9vMhrKPlPPXiOJEhGlGlAdiyxZvZxkCVI+v0zbhe21cY3/luLyxpSzzA==`（与 registry 元数据逐字符一致 ✅） |
| License | MIT |
| 上游仓库 | `github.com/deepseek-ai/deepseek-harness`（`apps/cli` 目录） |
| 入库日期 | 2026-08-16（rc.6）；2026-08-20 升级 rc.8 |
| 入库方式 | npm tarball 解压至本目录（`--strip-components=1`），未做任何源码修改 |

## 纪律

- 本目录**只读**：任何修改必须先复制出去再改，保持与上游 rc.8 字节一致，可审计。
- 运行时依赖解析仍走 pnpm（`pnpm add @deepseek-ai/dsh@0.1.0-rc.8` 于使用方包）；本目录是**审计基线 + 文档事实源**，升级前对照 diff。
